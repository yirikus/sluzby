            ...
            if (person == null) { //nobody assigned to the day
                ...
                for (int k = 5; k >= 0 && !assigned; k--) {
                    for (int j = 0; j < 32 && !assigned; j++) { //choose person with least jobs
                        for (Person p : persons) {
                            if (!p.isWantsOnly()) {
                                if (p.sizeOfWorkingDays() == j && p.canWorkOn(i) && !(d.isHoliday() && p.hasHoliday()) && !p.worksAround(i, k)) {
            ...
            if (person == null) { //nobody assigned to the day
                ...
                for (int k = 5; k >= 0 && !assigned; k--) {//choose person with least jobs around this date
                    for (int j = 0; j < 32 && !assigned; j++) { //choose person with least jobs
                        ...
                        for (Person p : persons) {
                            if (!p.isWantsOnly()) {
                                if (p.sizeOfWorkingDays() == j && p.canWorkOn(day.getInteger()) && !(day.isHoliday() && p.hasHoliday()) && !p.worksAround(day.getInteger(), k)) {
